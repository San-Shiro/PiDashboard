...
async function runTests() {
  ...
  ws.onopen = () => {
    ...
    setTimeout(() => {
      ...
      ws.send(JSON.stringify({ type: 'cmd', daemon: 'media', data: { action: 'pause' } }));
    }, 1000);
  };
  ws.onmessage = (event) => {
    ...
    if (msg.type === 'state' && msg.widget === 'media') {
      console.log('Media Playing:', msg.data.playing);
    }
  };
  setTimeout(() => { ws.close(); resolveTests(); }, 3000);
  await testPromise;
  process.exit(0);
}
runTests();
