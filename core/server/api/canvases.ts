...
export function registerCanvasesRoutes(router: Router) {
  // GET /api/canvases — list all
  router.get('/api/canvases', () => {
    const { list } = loadAllCanvases();
    return json({ canvases: list });
  });

  // POST /api/canvases — create new canvas
  router.post('/api/canvases', async (req) => {
  ...
  router.get('/api/canvases/:id', (req, params) => {
  ...
  // PUT /api/canvases/:id — save canvas with widgets
  router.put('/api/canvases/:id', async (req, params) => {
    const filename = resolveCanvasFile(params.id);
    ...
    if (body.is_active) {
      const activeFile = join(CANVASES_DIR, 'active.json');
      writeFileSync(activeFile, JSON.stringify(body, null, 2), 'utf8');
      daemonManager.reconcile(body);
    }
  ...
  router.delete('/api/canvases/:id', (req, params) => {
    const filename = resolveCanvasFile(params.id);
    if (!filename) return error('Canvas not found', 404);
  ...
  // POST /api/canvases/:id/apply — activate this canvas
  router.post('/api/canvases/:id/apply', (req, params) => {
    const result = activateCanvasById(params.id);
    if (!result.ok) return error('Canvas not found', 404);
    return json({ success: true });
  });

  // POST /api/canvases/:id/publish — save draft changes are expected to already be persisted,
  // then mark this canvas active and reload kiosk display.
  router.post('/api/canvases/:id/publish', (req, params) => {
