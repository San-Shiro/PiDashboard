...
export function validateCanvas(raw: any, registryIds: string[]): ValidationResult {
  ...
  raw.widgets = raw.widgets.filter((w: any) => {
    ...
    if (!registryIds.includes(w.widget_id)) {
      warnings.push(`Unknown widget '${w.widget_id}' removed from canvas`);
      return false;
    }
    ...
    if (w.layout.filter && typeof w.layout.filter === 'object') {
      ...
      if (f.blur !== undefined) f.blur = clamp(f.blur, 0, 50);
      ...
      const allowedKeys = ['blur', 'brightness', 'contrast', 'grayscale', 'saturate', 'sepia', 'opacity'];
      for (const key of Object.keys(f)) {
        if (!allowedKeys.includes(key)) {
          warnings.push(`Unknown filter property '${key}' stripped`);
          delete f[key];
        }
      }
    }

    if (w.layout.transition) {
      const validTransitionRe = /^[a-z-]+\s+[\d.]+s(?:\s+[a-z-]+)?$/i;
      if (typeof w.layout.transition !== 'string' || !validTransitionRe.test(w.layout.transition)) {
        warnings.push(`Invalid transition value stripped`);
        delete w.layout.transition;
      }
    }
