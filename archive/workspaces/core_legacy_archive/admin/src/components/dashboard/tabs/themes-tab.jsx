    ...
    <Card className="p-5">
      <div className="flex items-start justify-between mb-2">
        ...
        {!theme.is_builtin && (
          <button
            onClick={onDelete}
            className="p-1.5 rounded-lg transition-colors hover-surface-2"
      ...
      <div
        className="mt-4 pt-3 border-t animate-fade-in"
...
function CustomThemeBuilder({ onSave, onClose }) {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="relative rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto border shadow-2xl"
        style={{
          pointerEvents: "auto",
...
export default function ThemesTab() {
  const { activeTheme, themes, activateTheme, isDark, toggleDarkLight, createCustomTheme, deleteCustomTheme } =
    useTheme();
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <Card className="p-4 flex items-center justify-between">
        <div>
          ...
          >
            Currently: <strong>{activeTheme?.name || "Midnight Blue"}</strong>
          </p>
        </div>
        <button
          onClick={toggleDarkLight}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all hover:scale-105 active:scale-95 duration-200"
          style={{
        ...
        >
          ...
          {isDark ? "Switch to Light" : "Switch to Midnight"}
        </button>
      </Card>
