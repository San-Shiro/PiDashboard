...
function CanvasEditorShell() {
          ...
          if (s.mode === "move") {
            return {
              ...
              base_config: {
                ...
                x: Math.max(
                  0,
                  Math.min(CANVAS_W - (b.width || 0), Math.round(s.origX + dx))
                ),
                y: Math.max(
                  0,
                  Math.min(
            ...
            return {
              ...
              base_config: {
                ...
                width: Math.max(
                  40,
                  Math.min(CANVAS_W - b.x, Math.round(s.origW + dx))
                ),
                height: Math.max(
                  30,
                  Math.min(CANVAS_H - b.y, Math.round(s.origH + dy))
                ),
              },
            };
          }
  ...
  const addWidget = useMutation({
    mutationFn: async (widget_id) => {
      const r = await fetch(`/api/templates/${canvasId}/add-widget`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ widget_id }),
      });
      if (!r.ok) throw new Error("add widget");
      return r.json();
    },
    onSuccess: (data) => {
      if (data.instance) {
        // Append locally — don't re-fetch and overwrite local edits
        const manifest = data.instance.manifest || registry.find((w) => w.id === data.instance.widget_id) || {};
        setLocalWidgets((prev) => [...prev, { ...data.instance, manifest }]);
        setDirty(true);
      }
    },
  });
      ...
      setLocalWidgets((prev) =>
        prev.map((inst) => {
          ...
          return {
            ...
            base_config: changes.base_config
              ? { ...inst.base_config, ...changes.base_config }
              : inst.base_config,
            widget_config: changes.widget_config
              ? { ...inst.widget_config, ...changes.widget_config }
              : inst.widget_config,
          };
        })
      );
  ...
  return (
    ...
    >
      ...
      {/* Canvas area */}
      <main
        className="flex-1 flex items-start justify-center p-6 overflow-auto animate-fade-in"
        ref={containerRef}
      >
        <div
          ...
          {localWidgets.map((inst) => {
            ...
            return (
              <div
                ...
                onClick={(e) => {
                  if (!dragState.current) setEditingInstanceId(inst.id);
                }}
                style={{
                  position: "absolute",
                  left: base.x * scale,
